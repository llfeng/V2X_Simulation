...
{
    if (stage == 0) {
        EV_INFO << "PhyLayerVlc::initialize " <<  getFullPath() << std::endl;
...
void PhyLayerVlc::handleMessage(cMessage* msg)
{
    ...
    if (msg->isSelfMessage()) {
        ...
        EV_INFO << "llfeng phy recv upperlayer at PhyLayerVlc::handleMessage" << std::endl;
...
//        if (txNode == getModuleByPath("^.^.")->getFullName()) {
        std::string txNode = VlcMsg->getSenderModule()->getModuleByPath("^.^.")->getName();
               ...
               EV_INFO << "sender " << VlcMsg->getSenderModule()->getFullName() << " receiver " << getFullName() << std::endl;
        ...
        if( (a==b) || (a=="nicVlcTail" && b == "rsuNicVlc") || (a=="rsuNicVlc" && b=="nicVlcTail")){
            EV_INFO << "Discarding received AirFrameVlc within the same host: " << VlcMsg->getSenderModule()->getFullPath() << std::endl;
