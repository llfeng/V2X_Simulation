...
//#define EV_INFO \
    if (debug) EV_LOG(omnetpp::LOGLEVEL_INFO, nullptr) << "[deciderVlc] "
...
simtime_t DeciderVlc::processNewSignal(AirFrame* msg)
{
    ...
    EV_DETAIL << "llfeng recvPower:" << recvPower << " at DeciderVlc::processNewSignal " << std::endl;
    ...
    if (signal.smallerAtCenterFrequency(minPowerLevel)) {
        ...
        EV_DETAIL << "AirFrame: " << frame->getId() << " with recvPower (" << recvPower << " < " << minPowerLevel << ") -> AirFrame can't be detected by the radio; discarded at its end." << std::endl;
        ...
        cMessage *collisionMsg = new cMessage("RECVD ENERGY", MacToPhyInterface::ENERGY_CHECKED);
        phy->sendControlMsgToMac(collisionMsg);

        if (!currentSignal.first) {
            // NIC is not yet synced to any frame, so lock and try to decode this frame
            currentSignal.first = frame;
            EV_DETAIL << "AirFrame: " << frame->getId() << " with (" << recvPower << " > " << minPowerLevel << ") -> Trying to receive AirFrame." << std::endl;
            ...
            EV_DETAIL << "AirFrame: " << frame->getId() << " with (" << recvPower << " > " << minPowerLevel << ") -> Already synced to another AirFrame. Treating AirFrame as interference." << std::endl;
...
//    start = start + PHY_VLC_SHR / bitrate; // its ok if something in the training phase is broken
    ...
    EV_DETAIL << "sinrMin : " << sinrMin << endl;
    DeciderResult80211* result = 0;
//    if(airFrames.size() > 1){
    if(sinrMin < 10){
        EV_DETAIL << "$$$$$$$$$$$$$$$$$$interfere frame count :" << airFrames.size() << std::endl;
        result = new DeciderResult80211(false, 0, 0, 10, true);
    }else{
        EV_DETAIL << "$$$$$$$$$$$$$$$$$$interfere frame count :" << airFrames.size() << std::endl;
        result = new DeciderResult80211(true, 0, 0, 10, false);
    }
    ...
    if (frame->getUnderMinPowerLevel()) {
        ...
        if (frame == currentSignal.first) {
            ...
            if (result->isSignalCorrect()) {
                ...
                EV_DETAIL << "packet was received correctly, it is now handed to upper layer...\n";
                // go on with processing this AirFrame, send it to the Mac-Layer
                phy->sendUp(frame, result);
            }
            else {
                DeciderResult80211 *result80211 = (DeciderResult80211*)(result);
                if(result80211->isCollision()){
//                    EV_INFO << "DeciderVlc::processSignalEnd st4.2...\n";
                    cMessage *collisionMsg = new cMessage("COLLISION OCCUR", MacToPhyInterface::COLLISION_DETECTED);
                    phy->sendControlMsgToMac(collisionMsg);
                }else{
//                    EV_INFO << "DeciderVlc::processSignalEnd st4.3...\n";
                }
            ...
            EV_DETAIL << "drop it. DeciderVlc::processSignalEnd st3.2...\n";
