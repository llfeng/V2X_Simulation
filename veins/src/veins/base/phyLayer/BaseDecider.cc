...
simtime_t BaseDecider::processSignal(AirFrame* frame)
{
    ...
    ASSERT(frame);
    EV_DETAIL << "Processing AirFrame... at BaseDecider::processSignal" << endl;
    EV_TRACE << "Processing AirFrame..." << endl;

    switch (getSignalState(frame)) {
    case NEW:
//        EV_INFO << "NEW at BaseDecider::processSignal" << endl;
        return processNewSignal(frame);
    case EXPECT_HEADER:
        EV_DETAIL << "EXPECT_HEADER at BaseDecider::processSignal" << endl;
        return processSignalHeader(frame);
    case EXPECT_END:
//        EV_INFO << "END at BaseDecider::processSignal" << endl;
...
void BaseDecider::getValidChannelInfo(simtime_t_cref start, simtime_t_cref end, AirFrameVector& out)
{
    phy->getValidChannelInfo(start, end, out);
}
