...
void Mac1609_4::handleUpperMsg(cMessage* msg)
{
    ...
    EV_INFO << "llfeng Received a message from upper layer for channel " << thisMsg->getChannelNumber() << " Access Category (Priority):  " << ac << std::endl;
    ...
    if (num == 1 && idleChannel == true && chan == activeChannel) {
        ...
        if (nextEvent != -1) {
            if ((!useSCH) || (nextEvent <= nextChannelSwitch->getArrivalTime())) {
                ...
                EV_INFO << "llfeng Updated nextMacEvent:" << nextMacEvent->getArrivalTime().raw() << std::endl;
