...
std::vector<std::pair<double, double>> VehicleObstacleControl::getPotentialObstacles(const AntennaPosition& senderPos_, const AntennaPosition& receiverPos_, const Signal& s) const
{
    ...
    EV_DETAIL << "searching candidates for transmission from " << senderPos.info() << " -> " << receiverPos.info() << " (" << senderPos.distance(receiverPos) << "meters total)" << std::endl;
    ...
    for (auto o : vehicleObstacles) {
        ...
        EV_DETAIL << "checking vehicle in proximity of " << hp_approx.info() << " with height: " << h << " width: " << w << " length: " << l << endl;
        ...
        if (!std::isnan(p1d) && p1d > 0 && p1d < maxd) {
            ...
            while (true) {
                ...
                if (it->first == p1d) { // omit double entries
                    EV_DETAIL << "two obstacles at same distance " << it->first << " == " << p1d << " height: " << it->second << " =? " << h << std::endl;
                    break;
                }
                if (it->first > p1d) {
