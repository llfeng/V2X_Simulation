...
#include "veins-vlc/phy/ReaderPhyLayerVlc.h"
...
Define_Module(veins::ReaderPhyLayerVlc);

/* Used for the LsvLightModel */
bool ReaderPhyLayerVlc::mapsInitialized = false;
std::map<std::string, Photodiode> ReaderPhyLayerVlc::photodiodeMap = std::map<std::string, Photodiode>();
std::map<std::string, RadiationPattern> ReaderPhyLayerVlc::radiationPatternMap = std::map<std::string, RadiationPattern>();

void ReaderPhyLayerVlc::initialize(int stage)
{
    if (stage == 0) {
        EV_INFO << "ReaderPhyLayerVlc::initialize " <<  getFullPath() << std::endl;
...
unique_ptr<AnalogueModel> ReaderPhyLayerVlc::getAnalogueModelFromName(std::string name, ParameterMap& params)
{
...
unique_ptr<AnalogueModel> ReaderPhyLayerVlc::initializeEmpiricalLightModel(ParameterMap& params)
{
...
unique_ptr<AnalogueModel> ReaderPhyLayerVlc::initializeLsvLightModel(ParameterMap& params)
{
...
unique_ptr<Decider> ReaderPhyLayerVlc::getDeciderFromName(std::string name, ParameterMap& params)
{
    if (name == "DeciderVlc") {
        protocolId = VLC;
...
std::shared_ptr<Antenna> ReaderPhyLayerVlc::getAntennaFromName(std::string name, ParameterMap& params)
{
    ...
    error("ReaderPhyLayerVlc only knows about HeadlightAntenna and TaillightAntenna");

    return nullptr;
}

std::shared_ptr<Antenna> ReaderPhyLayerVlc::initializeAntennaHeadlight(ParameterMap& params)
{
...
std::shared_ptr<Antenna> ReaderPhyLayerVlc::initializeAntennaTaillight(ParameterMap& params)
{
    double photodiodeGroundOffsetZ = par("photodiodeGroundOffsetZ");
    double interModuleDistance = par("interModuleDistance");
    std::string radiationPatternId = par("radiationPatternId");
...
unique_ptr<AnalogueModel> ReaderPhyLayerVlc::initializeVehicleObstacleShadowingForVlc(ParameterMap& params)
{
...
unique_ptr<Decider> ReaderPhyLayerVlc::initializeDeciderVlc(ParameterMap& params)
{
...
void ReaderPhyLayerVlc::handleMessage(cMessage* msg)
{
    // self messages
    if (msg->isSelfMessage()) {
        ...
        EV_DETAIL << "llfeng phy recv upperlayer at ReaderPhyLayerVlc::handleMessage" << std::endl;
...
unique_ptr<AirFrame> ReaderPhyLayerVlc::createAirFrame(cPacket* macPkt)
{
    return make_unique<AirFrameVlc>(macPkt->getName(), AIR_FRAME);
}

unique_ptr<AirFrame> ReaderPhyLayerVlc::encapsMsg(cPacket* macPkt)
{
    ...
    EV_DETAIL << "AirFrame w/ id: " << frame->getId() << " reader encapsulated, bit length: " << frame->getBitLength() << " duration: " << duration << "\n";
...
simtime_t ReaderPhyLayerVlc::setRadioState(int rs)
{
    if (rs == Radio::TX) decider->switchToTx();
    return BasePhyLayer::setRadioState(rs);
}

simtime_t ReaderPhyLayerVlc::getFrameDuration(int payloadLengthBits) const
{
