...
#include "veins-vlc/phy/TagPhyLayerVlc.h"
...
Define_Module(veins::TagPhyLayerVlc);

/* Used for the LsvLightModel */
bool TagPhyLayerVlc::mapsInitialized = false;
std::map<std::string, Photodiode> TagPhyLayerVlc::photodiodeMap = std::map<std::string, Photodiode>();
std::map<std::string, RadiationPattern> TagPhyLayerVlc::radiationPatternMap = std::map<std::string, RadiationPattern>();

void TagPhyLayerVlc::initialize(int stage)
{
    if (stage == 0) {
        EV_INFO << "TagPhyLayerVlc::initialize " <<  getFullPath() << std::endl;
...
unique_ptr<AnalogueModel> TagPhyLayerVlc::getAnalogueModelFromName(std::string name, ParameterMap& params)
{

    if (name == "VehicleObstacleShadowingForVlc") {
...
unique_ptr<AnalogueModel> TagPhyLayerVlc::initializeEmpiricalLightModel(ParameterMap& params)
{
...
unique_ptr<AnalogueModel> TagPhyLayerVlc::initializeLsvLightModel(ParameterMap& params)
{
...
unique_ptr<Decider> TagPhyLayerVlc::getDeciderFromName(std::string name, ParameterMap& params)
{
    if (name == "DeciderVlc") {
        protocolId = VLC;
...
std::shared_ptr<Antenna> TagPhyLayerVlc::getAntennaFromName(std::string name, ParameterMap& params)
{
    ...
    error("TagPhyLayerVlc only knows about HeadlightAntenna and TaillightAntenna");

    return nullptr;
}

std::shared_ptr<Antenna> TagPhyLayerVlc::initializeAntennaHeadlight(ParameterMap& params)
{
...
std::shared_ptr<Antenna> TagPhyLayerVlc::initializeAntennaTaillight(ParameterMap& params)
{
    double photodiodeGroundOffsetZ = par("photodiodeGroundOffsetZ");
    double interModuleDistance = par("interModuleDistance");
    std::string radiationPatternId = par("radiationPatternId");
...
unique_ptr<AnalogueModel> TagPhyLayerVlc::initializeVehicleObstacleShadowingForVlc(ParameterMap& params)
{
...
unique_ptr<Decider> TagPhyLayerVlc::initializeDeciderVlc(ParameterMap& params)
{
...
void TagPhyLayerVlc::handleMessage(cMessage* msg)
{
    // self messages
    if (msg->isSelfMessage()) {
        ...
        EV_DETAIL << "llfeng phy recv upperlayer at TagPhyLayerVlc::handleMessage" << std::endl;
...
//        if (txNode == getModuleByPath("^.^.")->getFullName()) {
        std::string txNode = VlcMsg->getSenderModule()->getModuleByPath("^.^.")->getName();
               std::string a = VlcMsg->getSenderModule()->getModuleByPath("^.")->getFullName();
               int q = VlcMsg->getSenderModule()->getModuleByPath("^.")->getId();

               //EV_INFO << "id_pos:"<< ppp->pos << std::endl;
               std::string b = getModuleByPath("^.")->getFullName();
               EV_DETAIL << "sender " << VlcMsg->getSenderModule()->getFullName() << " receiver " << getFullName() << std::endl;
               //EV_INFO << "sender " << a << " receiver " << b << std::endl;
        //if (a == b) {
        //if (a == b || b == "nicVlcTail" || (a != "rsuNicVlc" && b == "nicVlcHead")) {
        if( (a==b) || (a=="nicVlcTail" && b == "rsuNicVlc") || (a=="rsuNicVlc" && b=="nicVlcTail")){
            EV_DETAIL << "Discarding received AirFrameVlc within the same host: " << VlcMsg->getSenderModule()->getFullPath() << std::endl;
...
unique_ptr<AirFrame> TagPhyLayerVlc::createAirFrame(cPacket* macPkt)
{
    return make_unique<AirFrameVlc>(macPkt->getName(), AIR_FRAME);
}

unique_ptr<AirFrame> TagPhyLayerVlc::encapsMsg(cPacket* macPkt)
{
    ...
    if(duration > 1.0){
        EV_WARN << "bit length too long" << std::endl;
    }
    ASSERT(duration > 0);
    ...
    EV_DETAIL << "mac pkt len:" << frame->getEncapsulatedPacket()->getBitLength() << std::endl;
    EV_DETAIL << "AirFrame w/ id: " << frame->getId() << " encapsulated, bit length: " << frame->getBitLength() << " duration: " << duration << "\n";

//    EV_INFO << "Sender's ownpos(" << ownPos.x << "," << ownPos.y << "," << ownPos.z << ")"<< endl;
//    EV_INFO << "Sender's orient(" << ownOrient.x << "," << ownOrient.y << "," << ownOrient.z << ")"<< endl;
//    EV_INFO << "Sender's otherpos(" << otherPos.x << "," << otherPos.y << "," << otherPos.z << ")"<< endl;

    return airFrame;
}

simtime_t TagPhyLayerVlc::setRadioState(int rs)
{
    if (rs == Radio::TX) decider->switchToTx();
    return BasePhyLayer::setRadioState(rs);
}

simtime_t TagPhyLayerVlc::getFrameDuration(int payloadLengthBits) const
{
