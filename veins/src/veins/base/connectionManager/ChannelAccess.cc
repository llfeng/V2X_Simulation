...
//    EV_INFO << "llfeng send to channel at ChannelAccess::sendToChannel" << endl;
    if (useSendDirect) {
//        EV_INFO << "llfeng send to channel useSendDirect at ChannelAccess::sendToChannel" << endl;
        // use Andras stuff
//        EV_INFO << "llfeng send to channel useSendDirect at ChannelAccess::sendToChannel name: " << i->first->nicPtr->getFullName() << endl;
        if (i != gateList.end()) {
...
//        EV_INFO << "llfeng send to channel not useSendDirect at ChannelAccess::sendToChannel" << endl;
        // use our stuff
...
void ChannelAccess::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj, cObject* details)
{
    if (signalID == BaseMobility::mobilityStateChangedSignal) {
        ...
        EV_DETAIL << getFullPath() << ", antennaPosition" << antennaPosition.getPositionAt(simTime()) << " sim_t:" << simTime() << std::endl;
//        EV_INFO << "getRad:"  << heading.getRad() << " antennaOffsetYaw: " << antennaOffsetYaw << endl;
//        EV_INFO << "llfeng obj full name: " << obj->getFullName() << endl;
//        EV_INFO << "mobility obj full name: " << mobility->getFullName() << endl;
//        EV_INFO << "llfeng move Nic " << getParentModule()->getName() << "cur mod "<<getName() << endl;
        if (isRegistered) {
            ...
            EV_WARN << "llfeng register Nic " << getParentModule()->getName() << "cur mod "<<getName() << endl;
