...
{
    EV_DETAIL << "start receiving new AirFrame " << frame << " from channel." << endl;
    ...
    filterSignal(frame);
    if(frame->getSignal().getAtCenterFrequency() == 0){
        ...
        handleAirFrameEndReceive(frame);
        return;
    }else if (decider && isKnownProtocolId(frame->getProtocolId())) {
        channelInfo.addAirFrame(frame, simTime());
        ASSERT(!channelInfo.isChannelEmpty());
...
void BasePhyLayer::handleAirFrameReceiving(AirFrame* frame)
{
    ...
    EV_DETAIL << "receiving new AirFrame " << frame << " from channel." << endl;
    Signal& signal = frame->getSignal();
...
*/
    ...
    EV_DETAIL << "Handed AirFrame with ID " << frame->getId() << " to Decider. Next handling in " << nextHandleTime - simTime() << "s." << endl;
...
{
    EV_DETAIL << "end receiving new AirFrame " << frame << " from channel." << endl;
    EV_TRACE << "End of Airframe with ID " << frame->getId() << "." << endl;

    simtime_t earliestInfoPoint = channelInfo.removeAirFrame(frame);
//    if(!validChannelInfo.isChannelEmpty()){
//        validChannelInfo.removeAirFrame(frame);
//    }
...
void BasePhyLayer::realSend(cMessage* msg){

    // build the AirFrame to send
    ASSERT(dynamic_cast<cPacket*>(msg) != nullptr);

    AirFrameVector airFrames;
    getChannelInfo(simTime(), simTime() + 0.001, airFrames);
    simtime_t busyTime = SignalUtils::getSignalEndTime(simTime(), airFrames);
    EV_DETAIL << "full path:" << getFullPath()<<std::endl;
    if(getFullPath().find("reader") != std::string::npos && busyTime > 0 && busyTime > simTime()){
        double randNum = dblrand()/4;
        EV_DETAIL << "busyTime: " << busyTime << ", tar_time: " << busyTime + randNum<< endl;
...
//            throw cRuntimeError("Error: message for sending received, but radio already sending");
            ...
            unique_ptr<AirFrame> frame = encapsMsg(static_cast<cPacket*>(msg->getContextPointer()));
            ...
            EV_DETAIL << "llfeng send message down at BasePhyLayer::handleUpperMessage" << endl;
            sendMessageDown(frame.release());
            if(getFullPath().find("tag") != std::string::npos){
                setRadioState(1);
            }
        }
    }
...
{
    cMessage *originMsg = new cMessage("RETRY_MSG", PHY_RETRY);
    originMsg->setKind(PHY_RETRY);
    originMsg->setContextPointer(msg);
    realSend(originMsg);
//    // check if Radio is in TX state
//    if (radio->getCurrentState() != Radio::TX) {
//        delete msg;
...
//
//
//    EV_INFO<< "llfeng send message down at BasePhyLayer::handleUpperMessage" << endl;
//    sendMessageDown(frame.release());
}
...
unique_ptr<AirFrame> BasePhyLayer::encapsMsg(cPacket* macPkt)
{
    ...
    EV_DETAIL << "AirFrame encapsulated, length: " << frame->getBitLength() << "\n";
...
//    double receiverGain = antenna->getReceiverGain(receiverPosition.getPositionAt(), receiverOrientation, senderPosition.getPositionAt());
//    double senderGain = senderPOA.antenna->getSenderGain(senderPosition.getPositionAt(), senderOrientation, receiverPosition.getPositionAt());

    double attenna_gain = antenna->getGain(senderPosition.getPositionAt(), senderOrientation, receiverPosition.getPositionAt(), receiverOrientation);

    // add the resulting total gain to the attenuations list
    EV_TRACE << "origin signal power: " << signal.getAtCenterFrequency() << endl;
    ...
    for (auto& analogueModel : analogueModels) {
        EV_DETAIL << "analogueModel" << std::endl;
...
simtime_t BasePhyLayer::getChannelState(){
    AirFrameVector airFrames;
    getChannelInfo(simTime(), simTime() + 0.0001, airFrames);
    simtime_t busyTime = SignalUtils::getSignalEndTime(simTime(), airFrames);
    if(busyTime > 0){
        EV_DETAIL << "busyTime is " << busyTime << std::endl;
    }
    EV_DETAIL << "full path:" << getFullPath()<<std::endl;
    return busyTime;
}
...
//        EV_WARN << "Switched radio while sending an AirFrame. The effects this would have on the transmission are not simulated by the BasePhyLayer!";
    ...
    EV_DETAIL << "llfeng switchTime:" << switchTime << " at BasePhyLayer::setRadioState" << endl;
...
void BasePhyLayer::getValidChannelInfo(simtime_t_cref from, simtime_t_cref to, AirFrameVector& out)
{
    validChannelInfo.getAirFrames(from, to, out);
}
