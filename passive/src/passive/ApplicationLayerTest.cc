...
#include "passive/ApplicationLayerTest.h"

#include "passive/ApplicationLayerTestMessage_m.h"

using namespace veins;
using namespace passive;

Define_Module(passive::ApplicationLayerTest);

void ApplicationLayerTest::initialize(int stage)
{ 
    DemoBaseApplLayer::initialize(stage);
    if (stage == 0) {
        EV_INFO << "llfeng init application"<< std::endl;
        sentMessage = false;
        lastDroveAt = simTime();
...
void ApplicationLayerTest::handleSelfMsg(cMessage* msg)
{
    if (ApplicationLayerTestMessage* wsm = dynamic_cast<ApplicationLayerTestMessage*>(msg)) {
        ...
        EV_INFO << "llfeng send sendDown(wsm->dup()); at ApplicationLayerTest::handleSelfMsg" << std::endl;
        sendDown(wsm->dup());
    ...
    if (mobility->getSpeed() < 1) {
        if (simTime() - lastDroveAt >= 10 && sentMessage == false) {
            ...
            if (dataOnSch) {
                ...
                EV_INFO << "llfeng send sendDown(wsm); at ApplicationLayerTest::handlePositionUpdate" << std::endl;
                sendDown(wsm);
            }
        }
    }
